...
export async function POST(request: Request) {
  ...
  const { data: material, error } = await supabase
    .from("materials")
    .select("id,storage_path,status,name,mime_type")
    .eq("id", materialId)
  ...
  try {
    ...
    const extracted = await extractDocument(
      new File([await file.arrayBuffer()], material.name, { type: material.mime_type }),
    );
    if (!extracted.text.trim()) return NextResponse.json({ error: "No text could be extracted from this file." }, { status: 422 });

    const chunks = chunkText(extracted.text);
    await supabase
      .from("materials")
      .update({ extracted_text: extracted.text, status: "processing" })
      .eq("id", materialId)
      .eq("user_id", userData.user.id);
